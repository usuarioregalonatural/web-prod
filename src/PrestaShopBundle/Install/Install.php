...
class Install extends AbstractInstall
{
    ...
    public function populateDatabase($entity = null)
    {
        ...
        // LEO_THEME dont load COMMON_DATA from xml file
...
//        unset($xml_loader);
        ...
        # LEO_THEME dont need copy default image because we have demo images
...
//        }
    ...
    public function configureShop(array $data = array())
    {
        ...
        # LEO_THEME update shop_name
	Context::getContext()->shop->name = $data['shop_name'];
        Context::getContext()->shop->update();
        Configuration::loadConfiguration();
        ...
        if (is_array($states) && count($states)) {
            foreach ($states as $key => $state) {
                if(isset($states_default[$key])){   // LEO_THEME FIX : Undefined offset
    ...
    public function installModules($module = null)
    {
        ...
        foreach ($modules as $module_name) {
            ...
            // LEO_THEME already installed module, Prestashop update module wrong
...
//            }
    ...
    {
        return true; // LEO_THEME dont load Fixtures database from xml file
    ...
    {
        return true; // LEO_THEME dont need install theme
        # LEO_THEME try to get current theme
        $theme_name = '';
//        $themes = glob(dirname(dirname(__FILE__)).'../../../themes/*/config/theme.yml');
        $themes = glob(_PS_ROOT_DIR_.'/themes/*/config/theme.yml');
        if(count($themes) > 1){
            foreach ($themes as $key => $value) {
                $temp_name = basename(substr($value, 0, -strlen('/config/theme.yml')));
                if($temp_name == 'classic'){
                    continue;
                }else{
                    $theme_name = $temp_name;
                }
            }
        }
