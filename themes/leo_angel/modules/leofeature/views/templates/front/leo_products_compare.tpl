...
{block name='content'}
	<section id="main">
	{capture name=path}{l s='Products Comparison' d='Shop.Theme.Global'}{/capture}
	<h1 class="page-heading">{l s='Products Comparison' d='Shop.Theme.Global'}</h1>
	{if $hasProduct}
			...
			<table id="product_comparison" class="table table-bordered table-responsive">
				<tr>
					<td class="td_empty compare_extra_information">
						...
						<span>{l s='Features:' d='Shop.Theme.Global'}</span>
					</td>
					...
					{foreach from=$products item=product name=for_products}
						...
						<td class="product-miniature js-product-miniature leo-productscompare-item product-{$product.id_product}" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">
							<div class="delete-productcompare clearfix">
								<a class="leo-compare-button btn delete" href="#" title="{l s='Remove from Compare' d='Shop.Theme.Global'}" data-id-product="{$product.id_product}"><i class="material-icons">&#xE872;</i>
								</a>
							</div>
				...
				{else}
					<tr>
						<td></td>
						<td colspan="{$products|@count}" class="text-center">{l s='No features to compare' d='Shop.Theme.Global'}</td>
					</tr>
				{/if}
	...
	{else}
		<p class="alert alert-warning">{l s='There are no products selected for comparison.' d='Shop.Theme.Global'}</p>
	{/if}
