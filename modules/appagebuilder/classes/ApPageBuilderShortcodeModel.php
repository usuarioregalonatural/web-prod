...
class ApPageBuilderShortcodeModel extends ObjectModel
{
    ...
    public function delete()
    {
        ...
        if ($result) {
            ...
            if ($this->id_appagebuilder) {
                $obj = new ApPageBuilderModel($this->id_appagebuilder);
                $obj->delete();
            }
        }
    ...
    public function getShortCodeContent($id_appagebuilder = 0, $is_font = 0, $id_lang = 0)
    {
        ...
        $where = ' WHERE ps.id_shop='.(int)$id_shop.' AND pp.id_appagebuilder='.(int)$id_appagebuilder;
        ...
        $sql = 'SELECT p.*, pl.params, pl.id_lang
                FROM `'._DB_PREFIX_.'appagebuilder` p
                    ...
                    LEFT JOIN `'._DB_PREFIX_.'appagebuilder_shortcode` pp ON (p.id_appagebuilder=pp.id_appagebuilder)
     ...
     * @return type
     */
    public function getAllItems($id_appagebuilder, $is_font = 0, $id_lang = 0)
    {
...
//        $id_profiles = (int)$profile['id_appagebuilder_profiles'];
        ...
        $sql = 'SELECT p.*, pl.params, pl.id_lang
                ...
                WHERE
                    ...
                    AND pp.id_appagebuilder = '.(int)$id_appagebuilder.'
    ...
    public static function getShortCode($shortcode_key)
    {
        ...
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'appagebuilder_shortcode` p
                INNER JOIN `'._DB_PREFIX_.'appagebuilder_shortcode_shop` ps on(p.`id_appagebuilder_shortcode` = ps.`id_appagebuilder_shortcode`) WHERE
