...
//        if (Context::getContext()->controller === null || (isset(Context::getContext()->controller->controller_type) && in_array(Context::getContext()->controller->controller_type, array('front', 'modulefront')))) {
    ...
    public function uninstall()
    {
        ...
        Tools::copy(_PS_MODULE_DIR_.$this->name.'/shortcode/backup/tinymce.inc.js', _PS_ROOT_DIR_.'/js/admin/tinymce.inc.js');
    ...
    public function hookActionAdminControllerSetMedia()
    {
        Media::addJsDef(
            array(
                ...
                // 'appagebuilder_listshortcode_url' => $this->context->link->getAdminLink('AdminApPageBuilderShortcode').'&get_listshortcode',
            )
        );
    ...
    public function buildShortCode($content)
    {
        $result = preg_replace_callback(
            ...
            function ($matches_tmp) {
                ...
                return $this->processShortCode($tmp[1][0]);
            },
            $content
        );
        return $result;
    ...
    public function hookDisplayLeoProfileProduct($params)
    {
        ...
        if (!isset($params['product'])) {
            ...
            if ($params['product']['productLayout'] != '') {
                $tpl_file = _PS_ALL_THEMES_DIR_._THEME_NAME_.'/details/' . $params['product']['productLayout'].'.tpl';
            } else {
...
//        }
    ...
    public function hookDisplayBackOfficeCategory($params)
    {
        ...
        if (Validate::isLoadedObject($category = new Category((int)Tools::getValue('id_category')))) {
            ...
            $id_category = Tools::getValue('id_category');
            $layouts = _PS_THEME_DIR_.'profiles/';
            if (is_dir($layouts)) {
                // $category_layouts = preg_grep('/^plist/', scandir(_PS_THEME_DIR_.'profiles/'));
    ...
    public function hookActionObjectProductUpdateAfter($params)
    {
        ...
        if (isset($id_product) && $id_product) {
            ...
            if ($result) {
                ...
                if ($aplayout != 'default') {
                    Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'appagebuilder_page` (`id_product`,`id_category`,`page`,`id_shop`) VALUES ('.(int)$id_product.',0,\''.$aplayout.'\','.(int)$id_shop.')');
                }
            }
                ...
                if ($extrafied) {
                    ...
                    foreach ($this->languages as $lang) {
                        ...
                        if ($checkExist) {
                            $sql = '';
                            foreach ($extrafied as $value) {
                                $sql .= (($sql=='')?'':',').pSQL($value).'="'.pSQL($tab_hooks[$value][$lang['id_lang']]).'"';
                            }
