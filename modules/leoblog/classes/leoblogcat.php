...
class Leoblogcat extends ObjectModel
{
    ...
    public static function findByRewrite($parrams)
    {
        ...
        if (isset($parrams['link_rewrite']) && $parrams['link_rewrite']) {
            ...
            $sql .= " AND link_rewrite = '".$parrams['link_rewrite']."'";
        ...
        if ($this->hasChild($parent)) {
            ...
            foreach ($data as $menu) {
                ...
                $output .= '<li id="list_'.$menu['id_leoblogcat'].'">
                    <input type="checkbox" value="'.$menu['randkey'].'" name="chk_cat[]" id="chk-'.$menu['id_leoblogcat'].'" '.$check.'/>
    ...
    public function genFontEndTree($parent, $level, $helper)
    {
        if ($this->hasChild($parent)) {
            ...
            foreach ($data as $menu) {
                ...
                if ($menu['id_leoblogcat'] != $parent) {
                    # validate module
                    if ($this->hasChild($menu['id_leoblogcat'])) {
