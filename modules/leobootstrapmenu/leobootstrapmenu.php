...
//        }
        ...
        if (Tools::isSubmit('editgroup') || Tools::isSubmit('submitGroup') || Tools::isSubmit('deletegroup') || Tools::isSubmit('duplicategroup') || Tools::isSubmit('addmenuproductlayout') || Tools::isSubmit('importgroup') || Tools::isSubmit('importwidgets') || Tools::isSubmit('addNewGroup') || Tools::isSubmit('exportgroup') || Tools::isSubmit('changeGStatus')) {
            // print_r('test');die();
            if (Tools::isSubmit('submitGroup') || Tools::isSubmit('deletegroup') || Tools::isSubmit('duplicategroup') || Tools::isSubmit('addmenuproductlayout') || Tools::isSubmit('importgroup') || Tools::isSubmit('importwidgets') || Tools::isSubmit('changeGStatus')) {
                // print_r('testaaa');die();
...
//        $full_width = array(array('id' => '', 'name' => $this->l('Boxed')), array('id' => 'fullwidth', 'name' => $this->l('Fullwidth')));
...
//        $action_addwidget = $this->base_config_url.'&liveeditor=1&do=addwidget';
    ...
    public function hookDisplayHeader()
    {
        ...
        $this->context->controller->registerStylesheet('modules-leoboostrapmenu-fancybox', 'modules/leobootstrapmenu/js/fancybox/jquery.fancybox.css', array('media' => 'all', 'priority' => 150));
        $this->context->controller->registerJavascript('modules-leoboostrapmenu-leoboostrapmenu', 'modules/leobootstrapmenu/js/leobootstrapmenu.js', array('position' => 'bottom', 'priority' => 150));
        $this->context->controller->registerJavascript('modules-leoboostrapmenu-js-fancybox', 'modules/leobootstrapmenu/js/fancybox/jquery.fancybox.js', array('position' => 'bottom', 'priority' => 150));
        $link = new Link();
    ...
    private function _postProcess()
    {
        ...
        } elseif (Tools::isSubmit('addmenuproductlayout')) {
            //DONGND:: add new menu item for product layout
            if (!$id_group = Tools::getValue('id_group')) {
                ...
                if ($megamenu->save()) {
                    ...
                    //DONGND:: create widget with each product layout for new menu item
                    $sql = 'SELECT a.*
                        FROM `'._DB_PREFIX_.'appagebuilder_details` as a
                        INNER JOIN `'._DB_PREFIX_.'appagebuilder_details_shop` ps ON (ps.`id_appagebuilder_details` = a.`id_appagebuilder_details`) WHERE ps.id_shop='.$id_shop;
    ...
    {
        //DONGND:: update size of filed menu class
        ...
        if (count($correct_group_title) >= 1 && count($correct_group_lang_table) < 1) {
            ...
            foreach ($list_group_title as $list_group_title_item) {
                ...
                foreach ($list_lang as $list_lang_item) {
                    Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'btmegamenu_group_lang`(`id_btmegamenu_group`,`id_lang`,`title`) VALUES('.$group_id.', '.(int)$list_lang_item['id_lang'].', "'.$group_title.'")');
                }
            }
        ...
        if (count($correct_title_fo) < 1) {
            ...
            foreach ($list_lang as $list_lang_item) {
                ...
                Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'btmegamenu_group_lang` SET `title_fo` =  "'.$group_title_fo.'" WHERE `id_lang` = '.(int)$list_lang_item['id_lang']);
            }
        }
        Configuration::updateValue('BTMEGAMENU_GROUP_AUTO_CORRECT_TITLE_FO', 1);
    }
    
    //DONGND:: auto correct update database
